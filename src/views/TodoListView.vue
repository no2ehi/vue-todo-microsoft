...
function addCategories(selectedEditTodo?: (number | null), category?: Category | null): void {
...
const paginationTodo = computed(() => {
    const startIndex = (Number(route.query.page) - 1) * paginationData.pageSize || 0;
    const endIndex = startIndex + paginationData.pageSize || paginationData.pageSize;

    if(sortTodos.value === 'ASC') {
        // ascending
        return pagedTodo.value = allTodos.value.sort((a, b) => a.text.localeCompare(b.text)).slice(startIndex, endIndex); 
    } else if (sortTodos.value === 'DESC') {
        // descending
        return pagedTodo.value =  allTodos.value.sort((a, b) => b.text.localeCompare(a.text)).slice(startIndex, endIndex);
    } else {
        return pagedTodo.value =  allTodos.value.sort((a, b) => b.id - a.id).slice(startIndex, endIndex);
    }
});
...
const updateTodos = () => {
    ...
    todosCompleted.value = pagedTodo.value.filter((task: Todo) => task.completed) || [];
}
...
watch(() => route.query, () => {
    if(route.query.search){
        ...
        const selectedCategories = (route.query.selectedCategories as string)?.split(',');
...
const titleComputed = computed(() => {
    return route.query.search  ? `Searching For: ${route.query.search}` : `To Do List`
});
        ...
        <div v-if="showEdit && selectedEditTodo" class="w-[360px] min-h-screen  bg-[#252423] p-4">
            ...
            <div class="transition-all flex flex-col  px-3 py-4 rounded bg-[#2d2b2a] hover:bg-[#3b3a39] text-white cursor-pointer mb-2">
                <div class="flex justify-between mb-2 relative">
                    ...
                    <div v-if="showListCategory" @mouseleave="toggleListCategory" class="absolute top-7 right-2 w-[150px] h-auto flex flex-col bg-[#0b0b0b] border border-blue-400 rounded-md py-2 px-4 shadow-md">

                        <div 
                        v-for="category in categories" 
                        @click="addCategories(selectedEditTodo?.id, category)"
                        :key="category.id" class="py-1.5 hover:text-blue-400">{{ category.name }}</div>
                    </div>
