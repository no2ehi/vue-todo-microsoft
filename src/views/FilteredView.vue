...
onMounted(() => {
    const selectedCategories = route.params.selectedCategories?.split(',');
...
watch(() => route.params.selectedCategories, () => {
    console.log()
    const selectedCategories = route.params.selectedCategories?.split(',');
    allTodos.value = todoStore.filterTodo(selectedCategories) || [];
    console.log('watch', selectedCategories)
    todos.value = allTodos.value.filter((task: Todo) => !task.completed) || [];
    todosCompleted.value = allTodos.value.filter((task: Todo) => task.completed) || [];
    loading.value = todoStore.loading;
})

const filteredTitle: ComputedRef<string[]> = computed(() => {
    if(route.params.selectedCategories.length > 0) {
        return route.params.selectedCategories.split(',');
    } else {
        return [];
    }
})
    ...
    <div class="w-4/5 flex bg-[#0b0b0b] overflow-none ">
        <div class="flex flex-col flex-grow pt-4 h-screen ">
            ...
            <div class="px-6">
                ...
                <h2 class="text-[#78bafd] text-xl ml-2">Filter for: 
                    <span v-for="(cateogry, index) in filteredTitle" :key="index">{{ filteredTitle.length > 1 ? `${cateogry} / ` : cateogry }}</span></h2>
            </div>
            ...
            <div class="overflow-y-scroll max-h-screen px-6 pb-[12rem]"> 
                <div v-for="todo in allTodos" class="transition-all flex items-center px-3 py-4 rounded bg-[#2d2b2a] hover:bg-[#3b3a39] text-white cursor-pointer mb-2" @click.self="showEditTodo(todo)">
