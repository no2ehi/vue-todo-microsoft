...
<template>
  <div
    @click="ShowSortTask"
    class="relative flex items-center cursor-pointer px-2 py-1 bg-stone-900 rounded transition-all hover:bg-stone-800"
  >
    ...
    <div
      v-if="showSort"
      @mouseleave="ShowSortTask"
      class="absolute top-9 right-0 w-[200px] h-auto flex flex-col bg-[#0b0b0b] border border-blue-300 rounded-md py-2 px-4 shadow-md"
    >
      <div
        v-for="list in listTypeSort"
        @click="selectTypeSort(list)"
        :key="list.id"
        class="py-1.5 text-stone-200 hover:text-blue-300 transition-all text-sm"
      >
        {{ list.name }}
      </div>
    </div>
